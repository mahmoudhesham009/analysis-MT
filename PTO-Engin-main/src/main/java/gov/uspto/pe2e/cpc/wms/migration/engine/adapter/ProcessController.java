...
import gov.uspto.pe2e.cpc.wms.migration.engine.repository.mysql.ValidationLogs;
import gov.uspto.pe2e.cpc.wms.migration.engine.repository.mysql.ValidationLogsRepository;
import gov.uspto.pe2e.cpc.wms.migration.engine.service.MigrationServices;
import gov.uspto.pe2e.cpc.wms.migration.engine.service.MigrationServicesImplTest;
...
public class ProcessController {
	...
	@Autowired
	ValidationLogsRepository valLogRepo;
	...
	public ResponseEntity<MigrationResponse> migration(@PathVariable int jobId) {
		...
		try {
			...
			resultJSONObject = migrationServices.getDataFromApi("enterprise/historic-process-instances/query",
					inputDate, "", "POST", inputDate.isSingleProcess()).get(0).getJsonObject();
			...
			if (!resultJSONObject.isEmpty()) {
				...
				for (Object object : dataArray) {
					...
					ValidationLogs logs=valLogRepo.findByJobIdAndprocessId(jobId, jsonObject.getJSONObject("jsonObject").optString("id"));
					if(logs!=null) {
						System.out.println("Skip "+jsonObject.getJSONObject("jsonObject").optString("id"));
						constant.decreseJmsCount();
						if(i==dataArray.length()-1) {
							//update status of job
							
							migrationJob.get().setStatus("migrationFinished");
							migrationJobRepoSpringImpl.saveAndFlush(migrationJob.get());
		...
		for (ProcessMigrationInfo processMigrationInfo : allProcessData) {
			...
			if (cloneExist && originalExist) {
				...
				for (String id : ids) {
					reqBody.put("processInstanceId", id);
					reqBody.put("finished", false);
					CloseableHttpResponse response = migrationServices.callApiCore(client,
							"enterprise/historic-tasks/query", reqBody, "POST");
					String result = EntityUtils.toString(response.getEntity(), "UTF-8");
